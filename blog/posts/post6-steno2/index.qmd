...
The Kaplan meier estimator on a plot is wonderful in portraying differences in survival times. Below is a plot showing the differences between intervention groups, 
But first we have to fit the data.

```{r}
#| label: surv fit

# Using event_num because the event variable doesnt work
fit <- survfit(Surv(time_yrs, event_num) ~ allo, data = steno2)

# Summarising, showing just the first 10 years
summary(fit, times=c(0:10)) 
```
...
surv_p1 <- ggsurvplot(fit,
           data = steno2,
           risk.table = TRUE,         # optional: show number at risk
           conf.int = TRUE,           # optional: confidence interval
