...
# Each patient gets a start (baseline) year between 2015 and 2018
...
A <- seq_along(ids) |>
  ...
  list_rbind()   # <- modern replacement for map_dfr(..., .id = NULL)
...
# Positive follow-up length: max rel_year on/after start
fu_lengths <- swim_states |>
  group_by(id) |>
  summarise(fu_max = max(rel_year, na.rm = TRUE), .groups = "drop")

# Order: longest positive follow-up at the top
...
sarc_points <- B_rel |>
  ...
  mutate(
    # Put point in the middle of the year interval
...
sarc_points <- sarc_points |> mutate(sarcopenia  = factor(sarcopenia , levels = sarc_levels, ordered = TRUE))
ggplot() +
  ...
  labs(
    y      = "Patient ID (ordered by positive follow-up)",
